...
public class V_UpdateClass {
    ...
    private void setFrameComponents(){
        ...
        JLabel idLabel = new JLabel("Professor ID: "+classId);
        ...
        StylingFunctions.buttonStyling(mainFrame,updateButton,200,450,100,50);
        updateButton.addActionListener((ae)->updateButtonListener(classSubject,classDescription,profName.toString()));
    ...
    private void updateButtonListener(JTextField classSubject,JTextArea classDescription,String profName){
        if(classSubject.getText().equals("")||classDescription.getText().equals("")||profName.equals("")){
            JOptionPane.showMessageDialog(mainFrame,"All the fields are Required","Update Failed",JOptionPane.ERROR_MESSAGE);
        }else{
            ClassInfos updatedClass = new ClassInfos(classId,classSubject.getText(),classDescription.getText(),profName);
